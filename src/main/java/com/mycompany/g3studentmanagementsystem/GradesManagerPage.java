...
public class GradesManagerPage extends JFrame implements ActionListener {
	...
	public GradesManagerPage() {
		...
		// STUDENT ID at index 0 is hidden - keeps ID available for DB lookup
		String[] columns = {
	...
	public void loadGradesFromDatabase() {
		...
		try (Connection con = ConnectionString.getConnection(); Statement stmt = con.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
